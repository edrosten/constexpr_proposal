...
\constexpr candidates. And, as discussed above, \code{nearbyint} fails the final criterion.
...
This proposal impacts the existing headers \cmath\ and \header{cstdlib}. There may be an effect on existing code due to declaring certain functions with weak dependence on the rounding mode as \constexpr. The precise effect cannot be determined until the issue of how to treat the rounding mode of \constexpr\ arithmetic operations has been resolved. Independent of this, the proposal does not amount to 
a pure library extension. The subtlety arises because affected functions must report errors by compilation failure in \constexpr\ contexts instead of writing an error code into a global variable.
...
apply \constexpr throughout \cmath\ in a consistent manner. Ideally, this consistency of approach should encompass prior work on \code{complex}, since it has already been proposed that \code{complex::norm} be declared \constexpr~\cite{AP-complex}. For this purpose, ideally one would
