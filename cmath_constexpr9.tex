...
\begin{enumerate}
	...
	\item [R8] Improve and correct some examples; correct the first criterion for selecting functions. Provide a better critique of the State of the Art. Amend the wording in line with the spirit of the 2021-05-17 LEWG Telecon.
	
	\item [R9] Refine the wording in line with the spirit of the 2021-10-15 LWG Telecon, facilitated by 
	adding a new definition [defns.nonconst.subexpr] and normative wording to [expr.const]. The first of
	the original changes to [library.c] has been removed following the 2021-10-22 LWG Telecon.
\end{enumerate}
...
To facilitate the primary wording changes, a new definition [defns.nonconst.subexpr] is added to [intro.defs], together with normative wording in [expr.const]. It is anticipated that these will be broadly useful, beyond the scope of this paper.
Following this, an extra statement is added to [library.c], indicating that if a (mathematical) function raises a floating-point flag other than \FEINEXACT, then it is a non-constant subexpression. Annex F of the C Standard is invoked to define precisely when floating-point exceptions should be raised, and the behaviour when NaNs and/or infinities are passed as arguments. Beyond this, a new feature-test macro is added to [support.limits.general]. The remaining changes amount to scattering \constexpr\ throughout the existing headers \cmath\ and \header{cstdlib}, according to the rules specified earlier. 

With $\code{std\!::\!is\_constant\_evaluated()}$ it may be possible to implement the desired behaviour as a pure library extension; previously, this was not the case. 
...
\begin{acknowledgments}
	...
	for encouragement and advice. Sincere thanks also to Richard Smith, Jens Maurer,
	Tim Song and Davis Herring
	for help with standardese and Geoffrey Romer, Hubert Tong and Jonathan Wakely for additional 
	feedback and help.
\end{acknowledgments}


\begin{thebibliography}{1}	
	\bibitem[n2176]{CLang} ISO/IEC 9899:2018 Standard for Programming Languages --- C
	
	\bibitem[P0595]{ConstEval} Richard Smith, Andrew Sutton and Daveed Vandevoorde, \code{std\!::\!is\_constant\_evaluated()}.
	
	\bibitem[GCC Behaviour]{Blessed} \url{https://godbolt.org/z/dMGxh61xj}	
	
	\bibitem[P0415R0]{AP-complex} Antony Polukhin, Constexpr for std::complex.
	
	\bibitem[N4885]{WorkingPaper} Thomas K\"oppe, ed., Working Draft, Standard for Programming Language C++.	
\end{thebibliography}
...
\subsection{Modification to ``Terms and definitions'' [intro.defs]}

\parbox{\textwidth}{
{\bf 3.35}
\newline
{\bf multibyte character \hfill [defns.multibyte]}
\newline
sequence of one or more bytes representing a member of the extended character set of either the source or the execution environment
\newline
[{\it Note 1 to entry: The extended character set is a superset of the basic character set (5.3). ---end note}]}

\highlight{\parbox{\textwidth}{
{\bf 3.36}
\newline
{\bf non-constant subexpression \hfill [defns.nonconst.subexpr]}
\newline
expression whose evaluation as a subexpression of an expression E prevents E
from being a core constant expression
}}
\parbox{\textwidth}{
{\bf \remove{\st{3.36}} \highlight{3.37}}
\newline
{\bf NTCTS \hfill [defns.ntcts]}
...
\hspace{-2.5ex} $^5$ An expression E is a core constant expression unless the evaluation of E, following the rules of the abstract machine ([intro.execution]), would evaluate one of the following:
\newline

\ldots
\newline

(5.27) --- an invocation of the \code{va\_arg} macro ([cstdarg.syn]).

\highlight{\parbox{\textwidth}{(5.28) --- a call to a library function, when the call is specified as a non-constant subexpression\hfill

\hspace{2.25em} ([defns.nonconst.subexpr]).}}


\subsection{Modification to ``The C standard library'' [library.c]}

\highlight{\parbox{\textwidth}{ \hspace{-2.5ex} $^3$
A call to a C standard library function is a non-constant
subexpression ([defns.nonconst.subexpr]) if it raises a floating-point exception other
than \FEINEXACT. 
The semantics of a call to a C standard library
function evaluated as a core constant expression are those
specified in Annex F of the C standard [Footnote: see also ISO/IEC 9899:2018 section 7.6.]
to the extent applicable to the floating-point types ([basic.fundamental]) that are parameter
types of the called function.
[Note: Annex F specifies the conditions under which floating-point exceptions are raised and the behavior when NaNs and/or infinities
are passed as arguments.]
[Note: Equivalently, a call to a C standard library function is a non-constant subexpression if \code{errno} is set when \code{math\_errhandling \& MATH\_ERRNO} is \code{true}.] }}
