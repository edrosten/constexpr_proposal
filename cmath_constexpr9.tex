...
\begin{enumerate}
	...
	\item [R9] Refine the wording in line with the spirit of the 2021-10-15 LWG Telecon, facilitated by adding a new definition [defns.nonconst.subexpr].
\end{enumerate}
...
To facilitate the primary wording changes, a new definition [defns.nonconst.subexpr] is added to [intro.defs].

This proposal adds an extra statement to [library.c], indicating that if a (mathematical) function raises a floating-point flag other than \FEINEXACT, then it is a nonconstant subexpression. With the advent of $\code{std\!::\!is\_constant\_evaluated()}$ it may be possible to implement the desired behaviour as a pure library extension; previously, this was not the case. Beyond this, Annex F of the C Standard is invoked, wherever possible, to define precisely when floating-point exceptions should be raised.
...
\begin{acknowledgments}
	...
	for encouragement and advice. Sincere thanks also to Richard Smith, Jens Maurer and
	Tim Song
...
{\bf 3.14}
\newline
{\bf constant subexpression \hfill [defns.const.subexpr]}
\newline
...
\parbox{\textwidth}{
{\bf \remove{\st{3.15}} \highlight{3.16}}
\newline
{\bf deadlock \hfill [defns.deadlock]}
...
to the extent applicable to a floating-point type (basic.fundamental).
