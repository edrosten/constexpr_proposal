...
\begin{enumerate}
	...
	\item [R9] Refine the wording in line with the spirit of the 2021-10-15 LWG Telecon---with input from the 
	2021-11-12 CWG Telecon---facilitated by 
	adding a new definition [defns.nonconst.libcall] and normative wording to [expr.const]. The first of
...
{\bf 3.36}
\newline
{\bf non-constant library call \hfill [defns.nonconst.libcall]}
\newline
...
\highlight{\parbox{\textwidth}{(5.28) --- a non-constant library call ([defns.nonconst.libcall]).}}
...
library call ([defns.nonconst.libcall]) if it raises a floating-point exception other
than \FEINEXACT. 
...
[Note: Equivalently, a call to a C standard library function is a non-constant library call if \code{errno} is set when \code{math\_errhandling \& MATH\_ERRNO} is \code{true}.] }}
