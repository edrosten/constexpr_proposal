...
simple. Both GCC and clang already support this to some extent. 
GCC already declares the majority of functions as \constexpr, such as
\code{std::sqrt}, \code{std::exp}, \code{std::lgamma}, etc.
While clang
does not it does offer some functions as 
builtins and is able to use it to perform compile time computations, constant
...
$+,-,\times,/$ and functions such as \code{exp}, \code{sqrt}, \code{cos},
\code{sin} and so on are excluded.
