...
performed using standard library facilities. From inspection of \cmath, it may
...
\FEINEXACT\ may be raised. However, the same issues are faced by the standard
arithmetic operators and these are nevertheless available for use in constant
...
criteria above ensures that the functions to be changes are in some sense
simple. Bearing in mind that both GCC and clang support this to some extent. 
GCC already declares the more complicated
functions \code{std::exp} and \code{std::sqrt} to be \constexpr. While clang
does not currently support \constexpr for \code{std::sqrt}, it does offer it as
a builtin and is able to use it to perform compile time computations, constant
propagation and so on. It therefore hoped that any burden implicit in this
proposal is minimal.
...
values of numeric types are elements of this set. 
