...
of a mathematical function in a \constexpr\ context then compilation fails. The question
as to whether compilation should so fail is unambiguously answered by deferring to the 
C standard's Annex F~\cite{CLang}.
...
\begin{enumerate}
	...
	\item [R8] Improve and correct some examples; correct the first criterion for selecting functions. Provide a better critique of the State of the Art. Amend the wording in line with the 2021-05-17 Library Evolution Telecon.
\end{enumerate}
...
\begin{thebibliography}{1}	
	...
	\bibitem[N4878]{WorkingPaper} Thomas K\"oppe, ed., Working Draft, Standard for Programming Language C++.	
\end{thebibliography}
...
\highlight{\hspace{-2.5ex} $^3$ A call to a C standard library function that, according to Annex F of the C standard raises a floating-point exception},
\highlight{is not a core constant expression [expr.const].}
