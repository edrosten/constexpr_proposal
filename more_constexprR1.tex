...
Let us deal with these in turn. A crucial point to make, which motivates much of what follows, is that it is not straightforward to define what one means by correct rounding of a mathematical function, due to the peculiarities of floating-point numbers. Consider two examples at opposite extremes. On the one hand, if multiplying a number by two produced something off by the last bit, this would be highly concerning! On the other, what does it mean for something like \code{std::sin(1e100)} to be correctly rounded? Whilst it is of course true that there is an unambiguous result to this call, we may well wonder how meaningful it is, since shifting the argument by a tiny amount can cause the output to change dramatically. Indeed, the granularity of floating-point numbers at the scale of \code{1e100} is such that $\sin x$ sweeps across its entire range from $-1$ to $+1$ many times as we go from one floating-point input to the next available one. In this sense there is a fuzziness to floating-point numbers. We now put in place a framework to allow a rigorous treatment of these issues.
...
$C_{x_n}$ includes the numbers $x_{n-1}$ and $x_{n+1}$ (which may be $\pm \infty$). Thus, each chart goes up to be doesn't quite touch the numbers on either side.
...
In this case, $\{x_{m}\}$ comprises just a single entry and our prescription demands an exact calculation. Secondly, take the example of $sin(1e100)$. In this case, $\{x_{m}\}$ comprises all charts labelled by every representable number within the range $-1 \leq y \leq 1$. Our prescription states that every one of these numbers is an equally valid output from a conforming implementation. This chimes with our intuition of how to make sense of \code{std::sin(1e100)}.
...
template<>
struct do_stuff<true>
{
  static void execute() { destroy_the_universe(); }
};

// Do I feel lucky?
do_stuff<static_cast<bool>(std::sin(1e100))>::execute();
\end{verbatim}
