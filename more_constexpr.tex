...
In [library.c] it is noted that \cmath\ makes available the facilities of
...
Since such consistency is not a requirement, some compilers generate code which may be
independent of the particular runtime C library in use.
{\bf But couldn't it be argued that by making the generated code independent of the runtime library, the compiler is guaranteeing consistency?! Or is it basically saying that consistency between optimized / unoptimized code is lost?}
For example, consider the following code
snippet, kindly brought to our attention by Richard Smith:
...
For such code, any value in the range [-1, 1] could be considered reasonable and so it would not be expected to be consistent across library implementations.  Nevertheless,
on clang (targeting
x64), the following code is emitted, demonstrating that the result is in actual fact generated
...
_Z1fv:
	...
	retq
\end{Verbatim}
On GCC, equivalent code is generated.

%This stark example of where translation time and runtime behaviour could be very different is also rather artificial. As such, and given that it is already existing practice for functions such as \code{std::sin} to be evaluated at compile time, we make the argument in this paper 

While it is desirable to have consistent behaviour between runtime and
translation time, the fact that many floating point operations are subject to the
runtime rounding mode means that this is generally not possible.
