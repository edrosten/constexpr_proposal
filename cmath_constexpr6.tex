...
\begin{enumerate}
	...
	\item [R6] Ensure the proposed change to [expr.const] references [c.math]; propose the addition of a feature-test macro; further improvements to highlighting.
\end{enumerate}
...
\begin{acknowledgments}
	...
	and Jens Maurer for additional feedback and help.
\end{acknowledgments}
...
\begin{thebibliography}{1}
	...
	\bibitem[N4820]{WorkingPaper} Richard Smith, ed., Working Draft, Standard for Programming Language C++.	
\end{thebibliography}
...
{\small (4.22)} --- a \emph{throw-expression} [expr.throw] or a dynamic cast [expr.dynamic.cast] or \code{typeid} [expr.typeid] expression

\hspace{3.5em} that would throw an exception; \remove{\st{or}}

{\small (4.23)} --- an invocation of the \code{va\_arg} macro [cstdarg.syn]\remove{\st{.}}\highlight{;or}

\highlight{{{\small (4.24)}} --- an invocation of a mathematical function in the standard library [c.math] that encounters a domain error}

\hspace{3.8em} \highlight{or overflow.}
...
\subsection{Modifications to ``Implementation Properties/General'' [support.limits.general]}

A new row is to be added to table [tab:support.ft]:

\vspace{1ex}

\highlight{
	\code{\_\_cpp\_lib\_constexpr\_math} \qquad \qquad 201902L \qquad \qquad\header{cmath} \header{cstdlib}
}
