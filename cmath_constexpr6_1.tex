...
\begin{enumerate}
	...
	\item [R6] Transfer proposed wording from [expr.const] to [library.c]; propose the addition of a feature-test macro; further improvements to highlighting.
\end{enumerate}
...
and additional overloads may be declared in this document, but the behaviour and the
preconditions (including any preconditions implied by the use of an ISO C restrict qualifier) are the same
unless otherwise stated.

\vspace{1ex}

\highlight{\hspace{-2.5ex} $^3$ A call to a C standard library function that encounters a domain error or overflow is not a core constant expression}
